...
public class Blackboard extends Environment implements Runnable {
	...
	private final int P1_CELDA_OCUPADA = 14;
	private final int P2_CELDA_OCUPADA = 15;
	...
	private final int MAX_PARTIDAS = 20;
	// Numero de partidas en caso de empate
	private final int MAX_PARTIDAS_RONDAS_EXTRA = 10;
    ...
    public boolean executeAction(String ag, Structure action) {
		...
		clearPercepts();
		if (! ag.equals("player1") && ! ag.equals("player2")) {
			logger.info("El agente: "+ag+" no esta validado para mover.");
		} else {
        try {
			if (estado == JUGANDO) {
					if (action.getFunctor().equals("put")) {
						...
						try {
							if (ag.equals("player1")) {
								// mover a personaje1
								model.put(PIEZA_P1,x,y);
							} else if (ag.equals("player2") ) {							
								model.put(PIEZA_P2,x,y);
							}
						} catch (Exception ex) {
						}
		...
		while(true) {
			...
			switch(estado) {
				case CHECK_FIN_JUEGO:
					...
					if (numPartidas == max_partidas) {
						...
						if (timer.cronometro(5000.0)) {
							...
							logger.info("La ultima partida comenzo: "+turnoUltimaPartida);
							turno = turnoUltimaPartida;
							turno = pasarTurno();
							turnoUltimaPartida = turno;
							logger.info("La nueva partida debe comenzar: "+turno);
				...
				case P2_JUEGA_DOS_VECES:
					...
					timer.start();
					break;
				case P1_CELDA_OCUPADA:
					logger.info("Player1 ha movido a una celda ocupada");	
					view.setFinPartida(P2_GANADOR);
					anotar(P2_GANADOR);
					estado = CHECK_FIN_JUEGO;
					timer.start();
					break;
				case P2_CELDA_OCUPADA:
					logger.info("Player2 ha movido a una celda ocupada");
					view.setFinPartida(P1_GANADOR);
					anotar(P1_GANADOR);
					estado = CHECK_FIN_JUEGO;
					timer.start();
	 ...
	 */
	 private void anotar(int tipo_estado) {
		 ...
		 if (modoJuego == MODO_JUGAR_A_GANAR) {
			// logger.info("Esta partida la inicio Player"+turnoUltimaPartida);
			 switch (tipo_estado) {
				  case P1_GANADOR:
				  	if (turnoUltimaPartida==1) {p1_wins+=2;} else {p1_wins+=3;}; 
					break;
				  case P2_GANADOR: 
				  	if (turnoUltimaPartida==2) {p2_wins+=2;} else {p2_wins+=3;}; 
					break;
				  case EMPATE:
					p1_wins++;
					p2_wins++; 
			 ...
			 switch (tipo_estado) {
				  case P1_GANADOR:
				  	if (turnoUltimaPartida==1) {p1_wins+=3;} else {p1_wins+=4;}; 
					break;
				  case P2_GANADOR: 
				  	if (turnoUltimaPartida==2) {p2_wins+=3;} else {p2_wins+=4;}; 
					break;
				  case EMPATE:
					// Nadie puntua 
					break; 
	...
	private boolean checkGanadorVertical(int tipoJugador) throws Exception {
		int cont = 0;
		// Verticales
		for(int fil = 0; fil < GSize && cont < 4; fil++){
			if(model.getData()[ultimaFicha.getX()][fil] == tipoJugador){
				view.addFichaGanadora(ultimaFicha.getX(), fil);
				cont++;
			} else {
	...
	private int pasarTurno() {
		//if (turno == TURNO_NO_ESPECIFICADO) {turno = TURNO_PLAYER1;};
    ...
    class BlackboardModel extends CustomModel {
        ...
        void put(int tipoPieza, int x, int y) throws Exception {
		   ...
		   if ((data[x][y] == PIEZA_P1 || data[x][y] == PIEZA_P2)) {
			   // Add error cell busy
			   // logger.info("Intento poner pieza: "+ tipoPieza + " en una posicion ocupada");
			   if (tipoPieza == PIEZA_P1) {
				   estado = P1_CELDA_OCUPADA;
				   //logger.info("Cambio estado a: "+ estado);
			   };
			   if (tipoPieza == PIEZA_P2) {
				   estado = P2_CELDA_OCUPADA;
				   //logger.info("Cambio estado a: "+ estado);
			   };
		   } else {
			   add(tipoPieza, x, y);
			   //logger.info("Pongo pieza: "+ tipoPieza + " en pos(" + x +","+ y +")");
		   }
		}
    ...
    class BlackboardView extends CustomView {
        ...
        public BlackboardView(BlackboardModel model) {
            super(model, "4 En Raya - SI 2018/2019", 200);
